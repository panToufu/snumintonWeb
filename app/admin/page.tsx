...
export default function AdminPage() {
  ...
  const [adminTab, setAdminTab] = useState<"calendar" | "daily" | "monthly" | "members" | "register">("daily"); 
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-2 md:p-8">
      ...
      {/* 🔥 [추가됨] 모바일에서 캘린더 글자 크기를 확 줄여주는 CSS */}
      ...
      <div className="max-w-[1400px] mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[85vh] flex flex-col relative">
        ...
        <div className="bg-slate-900 text-white">
          <div className="p-4 md:p-6 pb-4 flex justify-between items-center">
            ...
            <button 
              onClick={() => router.push('/')}
              className="px-3 py-2 md:px-5 md:py-2.5 bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 rounded-xl text-xs md:text-sm font-bold transition-colors flex items-center gap-2 border border-slate-700"
            >
        ...
        <div className="flex flex-1 flex-col md:flex-row min-h-0 bg-slate-50">
            ...
            <>
              <div className="w-full md:w-[55%] border-b md:border-b-0 md:border-r border-slate-200 p-3 md:p-6 overflow-y-auto bg-white custom-scrollbar">
                <div className="mb-2 md:mb-4">
                  <h2 className="font-bold text-slate-800 text-sm md:text-base">출석 체크용 캘린더</h2>
                </div>
              ...
              <div className="w-full md:w-[45%] p-4 md:p-6 overflow-y-auto custom-scrollbar bg-slate-50/50">
                {!selectedEventId ? (
                  <div className="h-40 md:h-full flex flex-col items-center justify-center text-slate-400">
                    <span className="text-3xl md:text-4xl mb-2 md:mb-4">👆</span>
                    <p className="font-medium text-sm">달력에서 일정을 선택해주세요.</p>
                  </div>
                ) : (
                  <div>
                    <div className="flex justify-between items-end mb-4 md:mb-6">
                      <div>
                        <span className="text-[10px] md:text-xs font-bold text-blue-500 bg-blue-50 px-2 py-1 rounded mb-1 md:mb-2 inline-block">
                      ...
                      <span className="text-xs md:text-sm font-bold text-slate-600 bg-white border border-slate-200 px-2 md:px-3 py-1.5 rounded-xl shadow-sm">
                    ...
                    <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                      {applicants.length === 0 ? (
                        <div className="p-8 text-center text-slate-400 text-sm">신청자가 없습니다.</div>
                      ) : (
                          ...
                          return (
                            <div key={app.id} className="flex items-center justify-between p-3 md:p-3.5 hover:bg-slate-50 transition-colors">
                              <div className="flex items-center gap-2 md:gap-3 min-w-0">
                                <span className="text-[10px] md:text-xs font-black text-slate-300 w-3 md:w-4">{i + 1}</span>
                                <span className="font-bold text-slate-800 text-xs md:text-sm truncate max-w-[60px] md:max-w-none">{app.user_name}</span>
            ...
            <div className="w-full p-2 md:p-8 overflow-y-auto">
              <div className="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-slate-200 w-full max-w-5xl mx-auto">
                ...
                <FullCalendar
                  ...
                  displayEventTime={false}
                  headerToolbar={{ left: 'title', center: '', right: 'prev,next' }} 
                  eventClick={handleEventClickForEdit} 
                />
              </div>
            </div>
          )}
          ...
          {/* ========================================= */}
          {/* 탭 3: 📊 월별 상세 출석부 (모바일 가로 스크롤 완벽 대응) */}
          {/* ========================================= */}
          {adminTab === "monthly" && (
              ...
              <div className="max-w-full mx-auto">
                <div className="flex items-center justify-between mb-4 md:mb-6 bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-slate-100">
                  ...
                  <div className="text-center">
                    <h2 className="text-lg md:text-2xl font-black text-slate-800">{currentYear}년 {currentMonth}월 상세 출석부</h2>
                  </div>
                ...
                {/* 🔥 min-w-[800px] 제거하고 min-w-max 적용하여 스크롤 깔끔하게 처리 */}
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-x-auto custom-scrollbar">
                  <table className="w-full text-xs md:text-sm text-center min-w-max border-collapse">
                    <thead>
                      <tr className="bg-slate-100 text-slate-500 font-bold border-b border-slate-200">
                        {/* 🔥 모바일에서 순위/이름 칸 너비 대폭 축소 (w-8, w-14) */}
                        <th className="p-2 md:p-3 sticky left-0 bg-slate-100 z-10 w-8 md:w-16 border-r border-slate-200">순위</th>
                        <th className="p-2 md:p-3 sticky left-8 md:left-16 bg-slate-100 z-10 w-16 md:w-24 text-left shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] border-r border-slate-200">이름</th>
                        <th className="p-2 md:p-3 w-12 md:w-20 text-blue-600 border-r border-slate-200">총 횟수</th>
                        {monthEventsList.map(ev => (
                          <th key={ev.id} className="p-1.5 md:p-2 min-w-[36px] md:min-w-[50px] border-r border-slate-100 bg-white">
                            <div className="flex flex-col items-center">
                              <span className="text-[8px] md:text-[10px] text-slate-400 font-medium">{ev.type === 'normal' ? '정규' : ev.type === 'lesson' ? '레슨' : '행사'}</span>
                              <span className="text-slate-800 font-black">{new Date(ev.start_at).getDate()}일</span>
                            </div>
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {monthlyRanking.map((stat, idx) => (
                        <tr key={stat.id} className="hover:bg-slate-50 transition-colors bg-white">
                          ...
                          <td className="p-2 md:p-3 text-left font-bold text-slate-800 sticky left-8 md:left-16 bg-white/95 backdrop-blur-sm z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] border-r border-slate-50 truncate max-w-[4rem] md:max-w-none">{stat.name}</td>
                  ...
                  {monthlyRanking.length === 0 && (
                     <div className="p-8 text-center text-slate-400 text-sm">등록된 부원 명단이 없습니다.</div>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="max-w-3xl mx-auto">
                <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100 mb-6 flex flex-col gap-3">
                  ...
                  <div className="flex gap-2 justify-end items-center mt-2">
                    <select value={newMemberType} onChange={(e) => setNewMemberType(e.target.value)} className="px-3 py-2 border rounded-xl text-sm font-bold">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="p-4 bg-slate-100 font-bold text-slate-600 text-sm">전체 명단</div>
                  <div className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
                    {members.map(m => (
                      <div key={m.id} className="flex justify-between items-center p-3 hover:bg-slate-50">
                        <div className="flex items-center gap-2"><span className="font-bold text-sm">{m.name}</span><span className="text-[10px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded font-bold uppercase">{m.user_type}</span></div>
                        <button onClick={() => handleDeleteMember(m.id)} className="text-xs text-red-400 px-2 py-1">삭제</button>
                      </div>
                    ))}
                  </div>
                </div>
